...
public class AlarmManagerProxy extends KrollProxy {
	...
	// Add some extra information so when the alarm goes off we have enough
	...
	return TiApplication.getInstance().getApplicationContext().getPackageManager().getLaunchIntentForPackage(TiApplication.getInstance().getApplicationContext().getPackageName()).getClass()
		.getName();
    }

    @Kroll.method
    public void cancelAlarmNotification(@Kroll.argument(optional = true) Object requestCode) {
	// To cancel an alarm the signature needs to be the same as the
	// submitting one.
	...
	// If seconds are provided but not years, we just take the seconds to
	// mean to add seconds until fire
	boolean secondBased = (args.containsKeyAndNotNull("second") && !args.containsKeyAndNotNull("year"));

	// If minutes are provided but not years, we just take the minutes to
	// mean to add minutes until fire
    ...
    private Intent createAlarmServiceIntent(KrollDict args, int requestCode) {
	...
	intent.putExtra("alarm_service_requestcode", requestCode);
	intent.putExtra("alarm_service_request_code", requestCode);
	...
	// Create the Alarm Manager
	AlarmManager am = (AlarmManager) TiApplication.getInstance().getApplicationContext().getSystemService(TiApplication.ALARM_SERVICE);
	Intent intent = createAlarmServiceIntent(args, intentRequestCode);
	PendingIntent sender = PendingIntent.getBroadcast(TiApplication.getInstance().getApplicationContext(), intentRequestCode, intent, PendingIntent.FLAG_UPDATE_CURRENT);
	am.cancel(sender);
	...
	AlarmManager am = (AlarmManager) TiApplication.getInstance().getApplicationContext().getSystemService(TiApplication.ALARM_SERVICE);
	Intent intent = createAlarmServiceIntent(args, requestCode);
